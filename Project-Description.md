...
Consider how the existing endpoints will work with the new game - you'll need to 
...
taken before the number was found.
...
Remember, you are documenting an API that another programmer may want to use as the basis for a web or mobile app. An api user should *not* need to read the source code
...
Your responses can be in paragraph form or bulleted lists. This document should be around 500 words long.
...
for Guess a Number.
 ...
 - **cancel_game**
    ...
    You could implement this by deleting the Game model itself, or add a Boolean field such as 'cancelled' to the model.
...
For example: "If the User has not made a move in an 
...
main.py as a template. Remember that you will need to pass parameters
